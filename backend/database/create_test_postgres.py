"""
create_test_postgres.py
───────────────────────
Creates a complex PostgreSQL test database: `test_ecommerce`

Run from the backend/ directory.
Requires psycopg2-binary and a running PostgreSQL server.
"""
...
try:
    ...
    print("Please install psycopg2-binary to run this script.")
    exit(1)

# Default postgres credentials - adjust if necessary for your local setup
DSN_DEFAULT = "postgresql://postgres:postgres@localhost:5432/postgres"
DB_NAME = "test_ecommerce"
DSN_NEW_DB = f"postgresql://postgres:postgres@localhost:5432/{DB_NAME}"
...
def create_database():
    print(f"Connecting to default postgres to create {DB_NAME}...")
    try:
        conn = psycopg2.connect(DSN_DEFAULT)
        ...
        print("You may need to create it manually or adjust the DSN in this script.")
        exit(1)
...
def main():
    ...
    try:
        conn = psycopg2.connect(DSN_NEW_DB)
        create_tables(conn)
        seed(conn)
        conn.close()
        print(f"Done! Database '{DB_NAME}' is ready.")
        print(f"You can connect using DSN: {DSN_NEW_DB}")
    except Exception as e:
